...
export function runAutoLayout(nodes, edges) {
  ...
  for (const node of nodes) {
    g.setNode(node.id, { width: node.measured?.width ?? NODE_LAYOUT_WIDTH, height: node.measured?.height ?? 150 });
  }
  ...
  return nodes.map(node => {
    ...
    return {
      ...node,
      position: {
        x: dagreNode.x - (node.measured?.width ?? NODE_LAYOUT_WIDTH) / 2,
        y: dagreNode.y - (node.measured?.height ?? 150) / 2,
      },
    };
  });
}
